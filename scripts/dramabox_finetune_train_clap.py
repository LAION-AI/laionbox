#!/usr/bin/env python3
"""
DramaBox Voice Cloning LoRA Fine-Tuning with Optional CLAP Auxiliary Loss

Extends dramabox_finetune_train.py with:
  - CLAP auxiliary loss pushing audio toward natural voice quality
  - Filtered dataset support via --index-file
  - Adaptive CLAP coefficient maintaining ~25% ratio to diffusion loss

The CLAP loss works by:
  1. Estimating x_0 from the velocity prediction (no_grad decode to waveform)
  2. Scoring the waveform with VoiceCLAP-small against pos/neg text
  3. Using the CLAP reward to modulate an x_0 reconstruction loss weight
  4. Gradients flow through x_0_pred → velocity prediction → model

Usage (CLAP-guided):
    accelerate launch --num_processes=8 scripts/dramabox_finetune_train_clap.py \
        --config configs/finetune_clap.yaml

Usage (ablation, no CLAP):
    accelerate launch --num_processes=8 scripts/dramabox_finetune_train_clap.py \
        --config configs/finetune_ablation.yaml --no-clap
"""
...
class DramaBoxFinetuneDataset(Dataset):
    ...
    def __init__(self, preprocessed_dir: str, mode_weights: dict = None,
        ...
        # Build flat sample list: each entry is (group_key, sample_indices)
        # During __getitem__, we pick one sample from the group and one mode
        self._build_items()
    ...
    def _build_items(self):
        ...
        DramaBox samples get all 3 modes when expand_all_modes is set.
        """
        self.items = []
        modes = list(self.mode_weights.keys())
        if self.expand_all_modes:
            # All 3 modes for DramaBox samples, single mode for Emolia
            for sample in self.all_samples:
                source = sample.get("source", "dramabox")
                if source in ("emolia", "augmented"):
                    # Emolia/augmented pairs have direction baked in; use fwd only
...
# ── CLAP Auxiliary Loss Setup ─────────────────────────────────────────

def setup_clap_models(full_checkpoint, device, dtype):
    """Load AudioDecoder + VoiceCLAP-small for CLAP auxiliary loss.
    ...
    logging.info("Loading AudioDecoder (VAE decoder + vocoder) for CLAP loss...")
    ...
    logging.info("Loading VoiceCLAP-small for CLAP loss...")
    ...
    logging.info("CLAP auxiliary models loaded successfully")
...
def parse_args():
    ...
    p = argparse.ArgumentParser(parents=[cfg_parser],
                                description="DramaBox Voice Cloning LoRA Training with CLAP Auxiliary Loss")
    ...
    p.add_argument("--clap-loss", action="store_true", default=clap_default,
                   help="Enable CLAP auxiliary loss (default)")
    p.add_argument("--no-clap", action="store_true", default=False,
                   help="Disable CLAP auxiliary loss (for ablation)")
    p.add_argument("--clap-target-ratio", type=float,
                   default=_y("clap_target_ratio", 0.25),
                   help="Target ratio of CLAP aux to diffusion loss (default 0.25)")
...
def main():
    ...
    if is_main:
        ...
        args_dict["_meta"] = {
            ...
            "script": "dramabox_finetune_train_clap.py",
            "pattern": "IC-LoRA 3-mode voice cloning + CLAP auxiliary loss",
            "clap_enabled": args.clap_loss,
        }
    ...
    ema_clap = 0.0
    ema_alpha = 0.95
    clap_target_ratio = args.clap_target_ratio

    if clap_enabled:
        if is_main:
            logging.info("Setting up CLAP auxiliary loss models...")
        ...
        if is_main:
            logging.info(f"CLAP positive text: {args.positive_text[:80]}...")
            logging.info(f"CLAP negative text: {args.negative_text}")
            logging.info(f"CLAP target ratio: {clap_target_ratio}")
    ...
    if is_main:
        ...
        logging.info(f"CLAP auxiliary loss: {'ENABLED' if clap_enabled else 'DISABLED'}")
        if args.overfit:
    ...
    accum_clap_aux = 0.0
    accum_clap_reward = 0.0
    accum_clap_pos = 0.0
    accum_clap_neg = 0.0
    accum_clap_coeff = 0.0
    clap_log_count = 0
    best_loss = float("inf")
    best_step = 0
    t0 = time.time()
    ...
    for micro_step in range(start_micro, total_micro_steps):
        ...
        with accelerator.accumulate(model):
            ...
            if clap_enabled:
                ...
                with torch.no_grad():
                    # 1. Recover x_0 estimate: x_0 = (x_t - v_pred * sigma) / (1 - sigma)
                    #    Actually: noisy = (1-s)*clean + s*noise, vel = noise - clean
                    #    So: x_0_pred = noisy_tgt - pred_tgt * sigma_exp
                    #    But more precisely: x_0 = (noisy - sigma * (noise - vel_pred)) ...
                    #    Simplify: vel = noise - clean, so clean = noise - vel
                    #    And noisy = (1-s)*clean + s*noise
                    #    From vel_pred: x0_pred = noise - pred_tgt (if we had noise)
                    #    Better: x_t = (1-s)*x_0 + s*eps, v = eps - x_0
                    #    => x_0 = x_t - s*v => x_0 = noisy_tgt - sigma_exp * pred_tgt
                    noisy_tgt_tokens = state.latent[:, :tgt_T]  # [B, tgt_T, 128]
                    x0_pred_tokens = noisy_tgt_tokens - pred_tgt.detach() * sigma_exp

                    # 2. Unpatchify to latent shape: [B, 8, T, 16]
                    ...
                    # 3. Decode: latent → mel → waveform (first sample in batch only)
                    # Cast to bf16 to match decoder weights
                    decoded_audio = audio_decoder_clap(x0_latent[0:1].to(dtype=dtype))
                    waveform = decoded_audio.waveform.squeeze(0).float()  # [channels, samples]
                    audio_sr = decoded_audio.sampling_rate

                    # 4. Convert to 16kHz mono for CLAP
                    wav_mono = waveform.mean(dim=0) if waveform.ndim > 1 else waveform
                    if audio_sr != 16000:
                        wav_mono = torchaudio.functional.resample(wav_mono, audio_sr, 16000)

                    # 5. CLAP embeddings
                    audio_emb = clap_model.encode_waveform(wav_mono.unsqueeze(0).to(device))
                ...
                # 6. Reward = pos_sim - neg_sim (want to maximize)
                clap_reward_val = clap_pos_sim - clap_neg_sim

                # 7. x0 reconstruction loss weighted by CLAP reward
                # When CLAP reward is low → stronger x0 regularization
                # When CLAP reward is high → lighter regularization
                x0_clean = patchifier.patchify(tgt_latent)  # ground truth
                # Use the tokens WITH grad (pred_tgt, not detached x0_pred_tokens)
                x0_pred_grad = noisy_tgt_tokens.detach() - pred_tgt * sigma_exp
                x0_recon_loss = ((x0_pred_grad - x0_clean.detach()).pow(2).mean(dim=-1)
                                 * loss_mask).div(loss_mask.mean().clamp(min=1e-6)).mean()

                # Scale: negative reward → higher weight; positive reward → lower weight
                reward_weight = torch.clamp(
                    torch.tensor(0.5 - clap_reward_val, device=device),
                    min=0.05, max=2.0
                )
                clap_aux = reward_weight * x0_recon_loss

                # 8. Adaptive coefficient to maintain ~25% ratio
                ema_diff = ema_alpha * ema_diff + (1 - ema_alpha) * loss.item()
                ema_clap = ema_alpha * ema_clap + (1 - ema_alpha) * clap_aux.item()
                if ema_clap > 1e-8:
                    adaptive_coeff = clap_target_ratio * ema_diff / ema_clap
                else:
                    adaptive_coeff = clap_target_ratio
                adaptive_coeff = min(adaptive_coeff, 1.0)  # cap

                total_loss = loss + adaptive_coeff * clap_aux
                clap_aux_val = clap_aux.item()

                accelerator.backward(total_loss)

                # Track for logging
                accum_total_loss += total_loss.item()
                accum_clap_aux += clap_aux_val
                accum_clap_reward += clap_reward_val
                accum_clap_pos += clap_pos_sim
                accum_clap_neg += clap_neg_sim
                accum_clap_coeff += adaptive_coeff
                clap_log_count += 1
            else:
        ...
        if is_opt_step and step % args.log_every == 0 and is_main:
            ...
            # Add CLAP metrics
            if clap_enabled and clap_log_count > 0:
                avg_clap_reward = accum_clap_reward / clap_log_count
                avg_clap_pos = accum_clap_pos / clap_log_count
                avg_clap_neg = accum_clap_neg / clap_log_count
                avg_clap_aux = accum_clap_aux / clap_log_count
                avg_clap_coeff = accum_clap_coeff / clap_log_count
                avg_total_loss = accum_total_loss / clap_log_count

                log_msg += (
                    f" | clap_reward={avg_clap_reward:.3f} "
                    f"(+{avg_clap_pos:.3f}/-{avg_clap_neg:.3f}) "
                    f"aux={avg_clap_aux:.4f} coeff={avg_clap_coeff:.3f}"
                )

                metric.update({
                    "clap_pos_sim": round(avg_clap_pos, 4),
                    "clap_neg_sim": round(avg_clap_neg, 4),
                    "clap_reward": round(avg_clap_reward, 4),
                    "clap_aux_loss": round(avg_clap_aux, 6),
                    "clap_coeff": round(avg_clap_coeff, 4),
